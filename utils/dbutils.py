...
# cvdDLvTX3DtTU3U6D-mmb9Is%iLMuK$!
class database:
    ...
    def query_cols(self, seq_id, col_list):
        ...
        if not seq_id is None:
            sql = "select {0} from {1}.{2} where serial_number={3}".format(", ".join(col_list), self.db, self.table,
                                                                           seq_id)
        conn = self.connect_to_db()
        cursor = conn.cursor()
        cursor.execute(sql)
    ...
    def import_file(self, filepath):
        ...
        for row in range(len(df_raw)):
            ...
            ld_credibility_rating = float(df_raw.loc[row, "Credibility_rating"])
            if np.isnan(df_raw.loc[row, "ad_count"]):
                ld_ad_count = None
            else:
                ld_ad_count = float(df_raw.loc[row, "ad_count"])

            if np.isnan(df_raw.loc[row, "ad_max_size"]):
                ld_ad_max_size = None
            else:
                ld_ad_max_size = float(df_raw.loc[row, "ad_max_size"])

            if np.isnan(df_raw.loc[row, "css_definitions"]):
                ld_css_definitions = None
            else:
                ld_css_definitions = float(df_raw.loc[row, "css_definitions"])

            if np.isnan(df_raw.loc[row, "page_rank"]):
                ld_page_rank = None
            else:
                ld_page_rank = float(df_raw.loc[row, "page_rank"])

            if np.isnan(df_raw.loc[row, "bitly_clicks"]):
                ld_bitly_clicks = None
            else:
                ld_bitly_clicks = float(df_raw.loc[row, "bitly_clicks"])

            if np.isnan(df_raw.loc[row, "bitly_referrers"]):
                ld_bitly_referrers = None
            else:
                ld_bitly_referrers = float(df_raw.loc[row, "bitly_referrers"])

            if np.isnan(df_raw.loc[row, "tweets"]):
                ld_tweets = None
            else:
                ld_tweets = float(df_raw.loc[row, "tweets"])

            if np.isnan(df_raw.loc[row, "delicious_bookmarks"]):
                ld_delicious_bookmarks = None
            else:
                ld_delicious_bookmarks = float(df_raw.loc[row, "delicious_bookmarks"])

            if np.isnan(df_raw.loc[row, "fb_clicks"]):
                ld_fb_clicks = None
            else:
                ld_fb_clicks = float(df_raw.loc[row, "fb_clicks"])

            if np.isnan(df_raw.loc[row, "fb_comments"]):
                ld_fb_comments = None
            else:
                ld_fb_comments = float(df_raw.loc[row, "fb_comments"])

            if np.isnan(df_raw.loc[row, "fb_likes"]):
                ld_fb_likes = None
            else:
                ld_fb_likes = float(df_raw.loc[row, "fb_likes"])

            if np.isnan(df_raw.loc[row, "fb_shares"]):
                ld_fb_shares = None
            else:
                ld_fb_shares = float(df_raw.loc[row, "fb_shares"])

            if np.isnan(df_raw.loc[row, "fb_total"]):
                ld_fb_total = None
            else:
                ld_fb_total = float(df_raw.loc[row, "fb_total"])

            if np.isnan(df_raw.loc[row, "alexa_linksin"]):
                ld_alexa_linksin = None
            else:
                ld_alexa_linksin = float(df_raw.loc[row, "alexa_linksin"])

            if np.isnan(df_raw.loc[row, "alexa_rank"]):
                ld_alexa_rank = None
            else:
                ld_alexa_rank = float(df_raw.loc[row, "alexa_rank"])

            if np.isnan(df_raw.loc[row, "commas"]):
                ld_commas = None
            else:
                ld_commas = float(df_raw.loc[row, "commas"])

            if np.isnan(df_raw.loc[row, "dots"]):
                ld_dots = None
            else:
                ld_dots = float(df_raw.loc[row, "dots"])

            if np.isnan(df_raw.loc[row, "exclamations"]):
                ld_exclamations = None
            else:
                ld_exclamations = float(df_raw.loc[row, "exclamations"])

            if np.isnan(df_raw.loc[row, "questions"]):
                ld_questions = None
            else:
                ld_questions = float(df_raw.loc[row, "questions"])

            if np.isnan(df_raw.loc[row, "spelling_errors"]):
                ld_spelling_errors = None
            else:
                ld_spelling_errors = float(df_raw.loc[row, "spelling_errors"])

            if np.isnan(df_raw.loc[row, "text_complexity"]):
                ld_text_complexity = None
            else:
                ld_text_complexity = float(df_raw.loc[row, "text_complexity"])

            if np.isnan(df_raw.loc[row, "smog"]):
                ld_smog = None
            else:
                ld_smog = float(df_raw.loc[row, "smog"])
            ...
            if np.isnan(df_raw.loc[row, "JJ"]):
                ...
                ld_JJ = float(df_raw.loc[row, "JJ"])

            if np.isnan(df_raw.loc[row, "NN"]):
                ld_NN = None
            else:
                ld_NN = float(df_raw.loc[row, "NN"])

            if np.isnan(df_raw.loc[row, "DT"]):
                ld_DT = None
            else:
                ld_DT = float(df_raw.loc[row, "DT"])

            if np.isnan(df_raw.loc[row, "VB"]):
                ld_VB = None
            else:
                ld_VB = float(df_raw.loc[row, "VB"])

            if np.isnan(df_raw.loc[row, "RB"]):
                ld_RB = None
            else:
                ld_RB = float(df_raw.loc[row, "RB"])

            if np.isnan(df_raw.loc[row, "num_ne"]):
                ld_num_ne = None
            else:
                ld_num_ne = float(df_raw.loc[row, "num_ne"])

            if np.isnan(df_raw.loc[row, "sum_ne"]):
                ld_sum_ne = None
            else:
                ld_sum_ne = float(df_raw.loc[row, "sum_ne"])
            ...
            if np.isnan(df_raw.loc[row, "X1"]):
                ...
                ld_X1 = float(df_raw.loc[row, "X1"])

            if np.isnan(df_raw.loc[row, "X2"]):
                ld_X2 = None
            else:
                ld_X2 = float(df_raw.loc[row, "X2"])

            if np.isnan(df_raw.loc[row, "X3"]):
                ld_X3 = None
            else:
                ld_X3 = float(df_raw.loc[row, "X3"])

            if np.isnan(df_raw.loc[row, "X4"]):
                ld_X4 = None
            else:
                ld_X4 = float(df_raw.loc[row, "X4"])

            if np.isnan(df_raw.loc[row, "X5"]):
                ld_X5 = None
            else:
                ld_X5 = float(df_raw.loc[row, "X5"])

            if np.isnan(df_raw.loc[row, "X9"]):
                ld_X9 = None
            else:
                ld_X9 = float(df_raw.loc[row, "X9"])

            if np.isnan(df_raw.loc[row, "Total"]):
                ld_Total = None
            else:
                ld_Total = float(df_raw.loc[row, "Total"])

            if np.isnan(df_raw.loc[row, "Leik"]):
                ld_Leik = None
            else:
                ld_Leik = float(df_raw.loc[row, "Leik"])

            if np.isnan(df_raw.loc[row, "Eijk"]):
                ld_Eijk = None
            else:
                ld_Eijk = float(df_raw.loc[row, "Eijk"])

            if np.isnan(df_raw.loc[row, "Tastle"]):
                ld_Tastle = None
            else:
                ld_Tastle = float(df_raw.loc[row, "Tastle"])

            if np.isnan(df_raw.loc[row, "Leik 3 4 6"]):
                ld_Leik_3_4_6 = None
            else:
                ld_Leik_3_4_6 = float(df_raw.loc[row, "Leik 3 4 6"])

            if np.isnan(df_raw.loc[row, "correction"]):
                ld_correction = None
            else:
                ld_correction = float(df_raw.loc[row, "correction"])
            ...
            if np.isnan(df_raw.loc[row, "troia_label"]):
                ...
                ld_troia_label = float(df_raw.loc[row, "troia_label"])
            ...
            cur.execute(sql_query, (
                li_serial_num, li_doc_id, ls_url, ld_credibility_rating, ld_ad_count, ld_ad_max_size,
                ld_css_definitions,
                ld_page_rank, ld_bitly_clicks, ld_bitly_referrers, ld_tweets, ld_delicious_bookmarks, ld_fb_clicks,
                ld_fb_comments, ld_fb_likes, ld_fb_shares, ld_fb_total, ld_alexa_linksin, ld_alexa_rank, ld_commas,
                ld_dots,
                ld_exclamations, ld_questions, ld_spelling_errors, ld_text_complexity, ld_smog, ls_category, ld_JJ,
                ld_NN,
